...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !hotkey.register(shortcut) { model.error = "Не удалось зарегистрировать сочетание. Выберите другое в настройках; панель доступна через значок ключа в строке меню." }
    ...
    private func buildMenu() {
        ...
        let menu = NSMenu()
        let show = NSMenuItem(title: "Показать код   \(shortcut.label)", action: #selector(togglePanel), keyEquivalent: "")
        show.target = self; menu.addItem(show)
        let accounts = NSMenuItem(title: "Аккаунты и настройки…", action: #selector(openManagement), keyEquivalent: ",")
        accounts.target = self; menu.addItem(accounts)
        menu.addItem(.separator())
        let quit = NSMenuItem(title: "Завершить NotchOTP", action: #selector(quitApp), keyEquivalent: "q")
        ...
        appSubmenu.addItem(withTitle: "Завершить NotchOTP", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    @objc private func openManagement() {
        ...
        if manager == nil {
            ...
            let window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 660, height: 560), styleMask: [.titled, .closable, .miniaturizable], backing: .buffered, defer: false)
    ...
    private func updateShortcut(_ value: Shortcut) {
        ...
        guard hotkey.register(value) else {
            model.error = "Это сочетание занято. Попробуйте другое; прежнее сочетание продолжает работать."
            return
        }
        shortcut = value
        ...
        statusItem.menu?.items.first?.title = "Показать код   \(value.label)"
    }
