...
struct QRImport {
    static func choose(completion: @escaping (Result<OTPAccount?, Error>) -> Void) {
        ...
        panel.title = L10n.text("Выберите изображение с QR-кодом")
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let result: Result<OTPAccount?, Error> = Result {
                guard let source = CGImageSourceCreateWithURL(url as CFURL, nil),
                    ...
                    throw OTPError.invalid(L10n.text("Не удалось открыть изображение. Выберите PNG или JPEG с QR-кодом."))
                }
    ...
    guard otpPayloads.count == 1, let payload = otpPayloads.first else {
        if otpPayloads.count > 1 { throw OTPError.invalid(L10n.text("На изображении несколько аккаунтов. Обрежьте его до одного QR-кода.")) }
        throw OTPError.invalid(L10n.text("QR-код TOTP не найден. Выберите более чёткое изображение кода из настроек безопасности сервиса. Пакетный перенос Google не поддерживается."))
    }
