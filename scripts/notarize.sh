...
signature=$(codesign -dv --verbose=4 "$stage/NotchOTP.app" 2>&1)
case "$signature" in
    *'Authority=Developer ID Application:'*) ;;
    *) printf 'Сборка подписана не Developer ID:\n%s\n' "$signature" >&2; exit 1 ;;
esac
...
status=$(/usr/bin/plutil -extract status raw "$output_path/app-notarization.json")
test "$status" = Accepted || { printf 'Apple вернула статус %s для app\n' "$status" >&2; exit 1; }
...
name="NotchOTP-v${version}-macOS-$(uname -m)"
...
status=$(/usr/bin/plutil -extract status raw "$output_path/dmg-notarization.json")
test "$status" = Accepted || { printf 'Apple вернула статус %s для dmg\n' "$status" >&2; exit 1; }
...
(cd "$output_path" && shasum -a 256 "$name.zip" "$name.dmg" > SHA256SUMS.txt)
